...
if uploaded_pdf:
    ...
    if st.button("Genera PDF LQR"):
        ...
        # ------------------------------------------------------------
        # BARCODE TARGA — VERSIONE COMPATIBILE STREAMLIT CLOUD
        # ------------------------------------------------------------
        ...
        # ------------------------------------------------------------
        # SC opzionale — VERSIONE COMPATIBILE STREAMLIT CLOUD
        # ------------------------------------------------------------
        if sc_value:
            SC_X1, SC_Y1 = 415, 160
            SC_X2, SC_Y2 = 570, 240
            ...
            sc_barcode = code128.Code128(
                sc_value,
                barHeight=sc_height * 0.8,
                barWidth=1
            )

            scale_sc = min(sc_width / sc_barcode.width, sc_height / sc_barcode.height) * 0.75

            x_sc = SC_X1 + (sc_width - sc_barcode.width * scale_sc) / 2
            y_sc = y_bar

            c.saveState()
            c.translate(x_sc, y_sc)
            c.scale(scale_sc, scale_sc)
            sc_barcode.drawOn(c, 0, 0)
            c.restoreState()
