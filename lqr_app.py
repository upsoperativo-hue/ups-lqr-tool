...
if uploaded_pdf:
    ...
    if st.button("Genera PDF LQR"):
        ...
        # ------------------------------------------------------------
        # FUNZIONE PER BARCODE VETTORIALE SCALATO
        # ------------------------------------------------------------
        def draw_scaled_barcode(c, value, box_x1, box_y1, box_x2, box_y2):
            box_w = box_x2 - box_x1
            box_h = box_y2 - box_y1

            # Genera barcode vettoriale
            ...
            # Scala per farlo stare nel box (con margine)
            scale = min(
                (box_w * 0.8) / bc.width,
                (box_h * 0.8) / bc.height
            )
        ...
        # ------------------------------------------------------------
        # BARCODE TARGA (vettoriale scalato)
        # ------------------------------------------------------------
        ...
        # ------------------------------------------------------------
        # SC opzionale (vettoriale scalato)
        # ------------------------------------------------------------
        if sc_value:
